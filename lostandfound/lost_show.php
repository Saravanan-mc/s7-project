...
$lostItems = [];
$filePath = getDataDirPath() . 'lost_items.json';
...
if (file_exists($filePath)) {
    ...
    $lostItems = json_decode($jsonContent, true);

    if (!is_array($lostItems)) {
        $lostItems = [];
    }
    usort($lostItems, function ($a, $b) {
...
<head>
    ...
    <title>View Lost Items - Lost & Found Hub</title>
        ...
        <div class="glass-card p-8 shadow-2xl">
            <div class="text-center mb-8">
                <div class="inline-flex items-center justify-center w-20 h-19">
                    <i class="fa-solid fa-broken-chain text-blue-600 text-4xl"></i>
                </div>
                <h2 class="text-4xl font-extrabold bg-gradient-to-r from-blue-600 to-blue-800 bg-clip-text text-transparent mb-3">Lost Items Reported</h2>
                <p class="text-gray-600 text-lg">Browse through items that have been reported lost by others.</p>
            </div>
            ...
            <?php if (empty($lostItems)): ?>
                <div class="bg-blue-50 border border-blue-200 text-blue-800 p-4 rounded-xl flex items-center justify-center gap-3">
                    ...
                    <p class="font-medium text-lg">No lost items posted yet. Be the first to report one!</p>
                </div>
            <?php else: ?>
                <div class="grid grid-cols-2 md:grid-cols-2 lg:grid-cols-1 gap-6">
                    <?php foreach ($lostItems as $item): ?>
                        <div class="item-card flex flex-col justify-between">
                            <div>
                                ...
                                <p class="text-gray-600 text-sm mb-1"><strong>Last Seen:</strong> <?php echo htmlspecialchars($item['location'] ?? 'N/A'); ?></p>
                                <p class="text-gray-600 text-sm mb-2"><strong>Date Lost:</strong> <?php echo htmlspecialchars($item['date'] ?? 'N/A'); ?></p>

                                <div class="contact-info">
                                    <h4><i class="fa-solid fa-address-card mr-2 text-primary"></i> Contact:</h4>
                                    <p class="text-gray-700 text-sm mb-1"><strong>Roll Number:</strong> <?php echo htmlspecialchars($item['roll_number'] ?? 'N/A'); ?></p>
                                ...
                                <?php if (!empty($item['media_path'])): ?>
                                    ...
                                    <div class="mb-4">
                                        <?php if ($mediaType === 'image'): ?>
                                            <img src="<?php echo $mediaUrl; ?>" alt="Lost Item Image" class="media-preview">
